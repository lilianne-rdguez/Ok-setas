document.addEventListener("DOMContentLoaded",()=>{const t=document.getElementById("mobile-menu-btn"),e=document.getElementById("mobile-menu"),n=t?.querySelector("span");t&&e&&n&&t.addEventListener("click",()=>{e.classList.toggle("hidden"),n.textContent=e.classList.contains("hidden")?"menu":"close"}),document.querySelectorAll(".search-input").forEach(a=>{a.addEventListener("keypress",o=>{o.key==="Enter"&&o.target.value.trim()!==""&&(window.location.href=`/productos?q=${encodeURIComponent(o.target.value.trim())}`)})})});function c(){if(typeof window>"u")return[];try{const t=localStorage.getItem("growtrades_cart");return t?JSON.parse(t):[]}catch{return[]}}function d(t){typeof window>"u"||(localStorage.setItem("growtrades_cart",JSON.stringify(t)),window.dispatchEvent(new CustomEvent("cart-updated",{detail:t})))}function u(t,e=1){const n=c(),i=n.find(a=>a.id===t.id);i?i.quantity+=e:n.push({...t,quantity:e}),d(n)}function m(t,e){let n=c();const i=n.find(a=>a.id===t);i&&(e<=0?n=n.filter(a=>a.id!==t):i.quantity=e,d(n))}function f(t){let e=c();e=e.filter(n=>n.id!==t),d(e)}function w(){return c().reduce((e,n)=>{const i=n.price.replace(",",".").replace(/[^\d.-]/g,""),a=parseFloat(i);return e+a*n.quantity},0)}window.openCart=()=>{const t=document.getElementById("cart-drawer-overlay"),e=document.getElementById("cart-drawer");!t||!e||(t.classList.remove("hidden"),setTimeout(()=>{t.classList.remove("opacity-0"),e.classList.remove("translate-x-full")},10),window.renderCart())};window.closeCart=()=>{const t=document.getElementById("cart-drawer-overlay"),e=document.getElementById("cart-drawer");!t||!e||(t.classList.add("opacity-0"),e.classList.add("translate-x-full"),setTimeout(()=>{t.classList.add("hidden")},300))};window.addToCartGlobal=t=>{u(t,t.quantity||1),window.openCart()};document.addEventListener("DOMContentLoaded",()=>{const t=document.getElementById("cart-drawer-overlay");document.getElementById("cart-drawer");const e=document.getElementById("close-cart"),n=document.getElementById("cart-items-container"),i=document.getElementById("cart-total");document.addEventListener("click",o=>{o.target.closest(".open-cart-btn")&&(o.preventDefault(),window.openCart())}),t?.addEventListener("click",window.closeCart),e?.addEventListener("click",window.closeCart),window.renderCart=()=>{const o=c();if(!n)return;o.length===0?n.innerHTML=`
                ...
                `).join("");const s=w();i&&(i.innerText=s.toLocaleString("es-ES",{minimumFractionDigits:2})+" €")},window.updateCartItem=(o,s)=>{m(o,s)},window.removeCartItem=o=>{f(o)},window.addEventListener("cart-updated",()=>{window.renderCart(),a()});const a=()=>{const s=c().reduce((r,l)=>r+l.quantity,0);document.querySelectorAll(".cart-badge").forEach(r=>{r.textContent=s.toString(),s>0?r.classList.remove("hidden"):r.classList.add("hidden")})};a()});
